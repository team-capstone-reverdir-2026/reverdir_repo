
# 📚 ProjectName
<br>

> 유저 간 펜팔을 통해 책 추천을 받고 개인 독서 내역을 기록하는 소셜형 독서 기록 서비스
<br><br>

---

## 🚀 Overview  
<br>

>유저 간 펜팔 형태의 소통을 통해
>책을 추천받고, 자신의 독서 경험을 기록할 수 있는 서비스입니다.
><br><br>
>기존 독서 앱의 단조로운 기록 방식에서 벗어나,  
>소셜 요소 + 게임 요소를 결합하여 독서를 더 재미있고 지속 가능한 활동으로 만드는 것을 목표로 합니다.

<br>

---

## 🎯 Target & Problem
### 👤 Target User
> - 독서를 습관화하고 싶은 사람
> - 기록을 꾸준히 남기기 어려운 사람

### ⚠️ Pain Point

> - 기존 독서 앱의 단조로운 UX
> - 번거롭고 지속하기 어려운 기록 과정
<br>

---

## 🗂️ Structure

<br><br>

---

## ✨ Key Features
<br>

> - 📬 유저 간 책 추천
> - 📖 독서 기록 관리
> - 🏆 게임화 요소
> - 🔍 기록 관리 편의성

<br>

---

## 👥 Team - Reverdir
<br>

> - 이은효
> - 전채연
> - Lyu Dongying
<br>

---
## 📚 Documentation
<br>

* 📌 [Team Ground Rule](https://github.com/team-capstone-reverdir-2026/reverdir_repo/blob/main/Team_Ground_Rule.md)
* 📌 [Team Notion](https://www.notion.so/2026-capstone-fd7d4a4683e182fd9cea81cca11a4b88)

<br>
